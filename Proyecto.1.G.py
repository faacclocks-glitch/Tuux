...
def save_tienda(tienda, mercado_id, db_path=None):
    with _connect(db_path) as conn:
        c = conn.cursor()
        c.execute('INSERT INTO tiendas (mercado_id, nombre, direccion, celular, giro) VALUES (?, ?, ?, ?, ?)',
                  (mercado_id, tienda.nombre, tienda.direccion, tienda.celular, tienda.giro))
        return c.lastrowid
